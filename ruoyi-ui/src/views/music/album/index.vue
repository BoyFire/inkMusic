  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="专辑状态" prop="albumStatus">
        ...
        >
          <el-option
            v-for="(item, index) in albumStatusOptions"
            :key="index"
            :label="item.label"
            :value="item.value"
    ...
    >
      ...
      <el-table-column label="歌手名" align="center" prop="singers">
      ...
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="专辑类型" prop="albumType">
          <el-radio-group v-model="form.albumType">
            ...
            <el-radio :label="'Album'">album</el-radio>
          </el-radio-group>
        </el-form-item>

        <el-form-item label="专辑简介" prop="albumDesc">
          <el-input
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      getAlbum(id).then((response) => {
        ...
        for (let index = 0; index < response.data.singers.length; index++) {
          if (this.simpleSinger.indexOf(response.data.singers[index]) === -1) {
            this.simpleSinger.push(response.data.singers[index]);
          }
        }
        this.open = true;
        this.title = "修改专辑";
    ...
    handleDelete(row) {
      ...
      this.$modal
        .confirm('是否确认删除专辑编号为"' + row.albumId + '"的数据项？')
        .then(function () {
          return delAlbum(ids);
        })
        .then(() => {
    ...
    handleStatusChange(row) {
      ...
      this.$modal
        .confirm('确认要"' + text + '"专辑id为"' + row.albumId + '"的专辑吗？')
        .then(function () {

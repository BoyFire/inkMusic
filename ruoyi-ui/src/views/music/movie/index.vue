  ...
  <div class="app-container">
    ...
    >
      ...
      <el-form-item label="上传者类型" prop="movieUpType" label-width="100px">
        ...
        >
          <el-option
            v-for="(item, index) in upTypeOptions"
            :key="index"
            :label="item.label"
            :value="item.value"
      ...
      <el-form-item
        ...
        prop="movieUpId"
        label-width="100px"
        v-if="this.queryParams.movieUpType != null"
      >
        <el-select
      ...
      <el-form-item label="标签状态" prop="movieStatus">
        ...
        >
          <el-option
            v-for="(item, index) in movieStatusOptions"
            :key="index"
            :label="item.label"
            :value="item.value"
            :disabled="item.disabled"
    ...
    >
      ...
      <el-table-column label="视频" align="center" prop="movieUrl">
        <template slot-scope="scope">
          <el-link v-bind:href="scope.row.movieUrl" target="_blank"
            >试看</el-link
          >
        </template>
      </el-table-column>
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item
          ...
          prop="movieUpId"
          label-width="100px"
          v-if="this.form.movieUpType != null"
        >
          <el-select
...
<script>
import {addMovie, delMovie, getMovie, listMovie, updateMovie,} from "@/api/music/movie";
import {selectSimpleSingerByName} from "@/api/music/singer";
import {selectSimpleUserByName} from "@/api/music/user";
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      listMovie(this.queryParams).then((response) => {
        ...
        for (let index = 0; index < response.rows.length; index++) {
          let movie = response.rows[index];
          if (movie.movieUpType == 1) {
            if (this.queryUser.indexOf(movie.movieUpId) === -1) {
              this.queryUser.push(movie.movieUpId);
            }
          } else {
            if (this.querySinger.indexOf(movie.movieUpId) === -1) {
              this.querySinger.push(movie.movieUpId);
            }
          }
        }
    ...
    handleStatusChange(row) {
      ...
      this.$modal
        .confirm('确认要"' + text + '""' + row.movieName + '"吗？')
        .then(function () {
    ...
    remoteSinger(params) {
      ...
      selectSimpleSingerByName(params).then((response) => {
        for (let index = 0; index < response.data.length; index++) {
          if (this.formSinger.indexOf(response.data[index]) === -1) {
            this.formSinger.push(response.data[index]);
          }
        }
      });
    ...
    remoteUser(params) {
      this.remoteLoading = true;
      this.formUser = [];
      selectSimpleUserByName(params).then((response) => {
        for (let index = 0; index < response.data.length; index++) {
          if (this.formUser.indexOf(response.data[index]) === -1) {
            this.formUser.push(response.data[index]);
          }
        }
      });
      this.remoteLoading = false;

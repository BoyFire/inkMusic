...
<script lang="ts" setup>
  ...
  const mId = ref("0");
  const mvDetail: Ref<any> = ref({});
  // 0: 歌曲 1: mv 2: 歌单 3: 专辑  4: 电台 5: 视频 6: 动态
  const type: Ref<number> = ref(1);
  const videoUrl: Ref<string> = ref("");
  const videoCover: Ref<string> = ref("");
  const simiMv: Ref<any> = ref([]);
  ...
  const getMvDetail = async () => {
    ...
    videoCover.value = res.data.cover;
  };
  ...
  onBeforeRouteUpdate((to) => {
    mId.value = to.query.id as string;
    init();
  });

  onMounted(() => {
    mId.value = route.query.id as string;
    init();

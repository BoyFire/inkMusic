              ...
              <div
                ...
                <div class="simi-song-status">
                  ...
                  <i
                    ...
                    :class="['iconfont', playSimiIcon(simiItem)]"></i>
...
<script lang="ts" setup>
  ...
  const songInfo: Ref<any> = ref(null);
  const sId = ref("0"); // 歌曲的ID
  const coverDesc = ref("");
  const simiSong: Ref<any> = ref([]);
  const playlists: Ref<any> = ref([]);
  const mlogMv: Ref<any> = ref([]);
  ...
  const isCurSong = computed(
    () =>
      isPlayed.value && curSongInfo.value && curSongInfo.value.id === sId.value
  );
  ...
  const songDisable = computed(() =>
    songInfo.value.license || songInfo.value.vip ? "disable" : ""
  );
  ...
  onBeforeRouteUpdate((to) => {
    sId.value = to.query.id as string;
    init();
  });
  onMounted(() => {
    sId.value = route.query.id as string;
    init();
  });
  const init = () => {
    if (sId.value) {

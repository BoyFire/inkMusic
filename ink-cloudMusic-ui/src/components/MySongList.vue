      ...
      <div class="list-main" :style="curSongSty">
        <div
          ...
          <div class="columnSong songlist-name">
            <router-link
              :class="typeSize"
              :to="{ path: '/song', query: { id: item.songId } }">
              {{ item.songName }}
            </router-link>
          </div>
          ...
          <div class="columnSinger songlist-singer">
            <!--- 返回的数据里用户的uid有可能为0 --->
            <router-link
              :to="{ path: '/artist/detail', query: { id: author.singerId } }"
              class="song_name"
              v-for="(author, k) in item.singers"
              :key="author.singerName"
              >{{
                k !== 0 ? " / " + author.singerName : author.singerName
              }}</router-link
            >
          </div>
          <!-- 专辑名 -->
          <div class="columnAlbum" v-if="typeSize !== 'mini'">
            <router-link
              ...
              :to="{ path: '/album', query: { id: item.albums[0].albumId } }"
              v-if="item.albums"
              >{{ item.albums[0].albumName }}</router-link
            >
          </div>
          <!-- 时长 -->
          <div class="columnTime">
            <div class="songlist-time">
              {{ $utils.formatSongTime(item.songDuration) }}
            </div>
...
<script lang="ts" setup>
  ...
  const songList: Ref<any> = ref(props.songList);
  ...
  const likeSong = async (item) => {
    const { data: res } = await proxy.$http.disCollectSong({
      songListId: songListId.value,
      songId: item.songId,
    });
    if (res.code != 200) {
      return proxy.$msg.error("取消收藏失败");
    }
    proxy.$msg.success("取消收藏成功");

    songList.value.splice(songList.value.indexOf(item), 1);
    emit("func", songList.value.length);
  };
  ...
  onMounted(() => {
    songListId.value = route.query.id as string;
  });
  // 获取歌曲列表

...
<template>
  <el-skeleton :loading="loading" animated :count="num" :throttle="500">
    ...
    <template #default>
      <div class="playlist">
        <div class="item" v-for="item in playList" :key="item.songListId">
          <router-link
            :to="{ path: '/playlist/Detail2', query: { id: item.songListId } }"
            class="faceImg">
            <el-image :src="item.songListImgUrl" fit="cover">
            ...
            <span class="playCount"
              ><i class="iconfont icon-playnum"></i> {{ item.count + "首" }}
            </span>
          </router-link>
